...
const Search = () => {
  ...
  return (
    <div ref={searchRef} onMouseDown={handleMouseDown} className="relative">
      ...
      {clickedInside && query.trim() !== "" && finishedQuery && (
        <div className="">
          {results.length > 0 ? (
            <ul className="absolute bg-white border border-gray-300 mt-1 w-full max-h-96 overflow-y-auto z-40">
              {results.map((product) => (
                <li key={product.id}>
                  <Link
            ...
            <p className="absolute bg-gray-50 border border-gray-300 mt-1 w-full z-40 p-4 font-medium">
              No results found
            </p>
          )}
        </div>
